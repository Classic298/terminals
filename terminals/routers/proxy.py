...
async def _proxy_request(
    ...
    for attempt in range(max_retries):
        try:
            ...
            # The tracked instance isn't reachable — it may have been reaped,
            # replaced by another worker, or restarted with a new port. Stop
            # trusting it and re-resolve so the retry targets the current
            # container (or provisions a fresh one). The final attempt only
            # invalidates: the instance must stay tracked so the idle reaper
            # can still clean up a wedged container.
            backend.invalidate_status(user_id, policy_id)
            if attempt < max_retries - 1:
                logger.debug("Proxy attempt {} to {} failed ({}), retrying...", attempt + 1, target_url, e)
                await asyncio.sleep(1)
                backend.drop_instance(user_id, policy_id)
                try:
                    instance = await _resolve_instance(
                        request, user_id, policy_id=policy_id, spec=spec,
                    )
...
async def _ws_proxy(
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            # Unreachable (or answering but not upgrading yet, e.g. still
            # initializing) — stop trusting the tracked instance and re-resolve
            # so the retry targets the current (or a fresh) container. The
            # final attempt only invalidates, keeping the instance tracked
            # for the idle reaper.
            backend.invalidate_status(user_id, policy_id)
            if attempt < max_retries - 1:
